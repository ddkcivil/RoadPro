                        ...
                        <Stack spacing={3} mt={2}>
                            ...
                            <Box>
                                ...
                                <CommentsPanel 
                                    ...
                                    users={[]} /* Temporary empty users */
                                    currentUser={{ id: 'current-user', name: 'Current User' }} /* Temporary current user */
                                    ...
                                    onLikeComment={(commentId) => {
                                        ...
                                        const updatedDocs = (project.documents || []).map(d => {
                                            if (d.id === previewDoc.id && d.comments) {
                                                const updatedComments = d.comments.map(c => {
                                                    if (c.id === commentId) {
                                                        const newLikes = c.likes || [];
                                                        const userIndex = newLikes.indexOf('current-user');
                                                        if (userIndex === -1) {
                                                            return { ...c, likes: [...newLikes, 'current-user'] };
                                                        } else {
                                                            return { ...c, likes: newLikes.filter(id => id !== 'current-user') };
                                                        }
                                                    }
                                                    return c;
                                                });
                                        ...
                                        // Update preview doc
                                        if (previewDoc && previewDoc.comments) {
                                            const updatedPreviewComments = previewDoc.comments.map(c => {
                                                if (c.id === commentId) {
                                                    const newLikes = c.likes || [];
                                                    const userIndex = newLikes.indexOf('current-user');
                                                    if (userIndex === -1) {
                                            ...
                                            setPreviewDoc({ ...previewDoc, comments: updatedPreviewComments });
                                        }
                                    }}
                                />
                            </Box>
