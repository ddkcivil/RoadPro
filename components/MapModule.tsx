      ...
      if (project.kmlData && project.kmlData.length > 0) {
        project.kmlData.forEach((kmlData: KMLData) => {
          try {
            ...
            if (typeof L.ogc === 'undefined' || typeof L.ogc.kml === 'undefined') {
              // Fallback to omnivore if available
              if (window && (window as any).omnivore) {
                const kmlLayer = (window as any).omnivore.kml.parse(kmlData.content);
                kmlLayer.addTo(map);
              ...
              // Use L.ogc.kml if available
              const kmlLayer = L.ogc.kml(null, { 
                kmlString: kmlData.content,
                async: false
              });
              kmlLayer.addTo(map);
          ...
          if (mapRef.current && kmlContent) {
            try {
              ...
              if (typeof L.ogc === 'undefined' || typeof L.ogc.kml === 'undefined') {
                // Fallback to omnivore if available
                if (window && (window as any).omnivore) {
                  const kmlLayer = (window as any).omnivore.kml.parse(kmlContent);
